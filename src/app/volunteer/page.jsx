...
export const metadata = {
  title: 'Volunteers',
  openGraph: {
    title: 'Volunteers | Hong Kong Open Source Conference 2026',
  },
};

function SocialLinks({
  website, github, x, mastodon, linkedin,
}) {
  const links = [
    {
      href: website,
      ...
      <header className="hero is-medium">
        <div className="hero-body">
          ...
          <p className="subtitle">
            HKOSCon 2026 is made possible by our dedicated volunteers.
          </p>
        </div>
      </header>
      <main className="container">
        {volunteers.map(({ role, members }) => (
          <section className="section" key={role}>
            <h2 className="title is-4">{role}</h2>
            {members.length === 0 ? (
              <p className="has-text-grey">To be announced</p>
            ) : (
              <div className="columns is-multiline">
                {members.map(({
                  name, website, github, x, mastodon, linkedin,
                }) => (
